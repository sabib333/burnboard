...
export function isProfane(text) {
  if (!text || typeof text !== 'string') return { profane: false };
  const cleaned = isClean(text);
  if (!cleaned) {
    return { profane: true, reason: 'Contains prohibited hate speech or slurs' };
  }
  return { profane: false };
}

export default { badWords, isClean, isProfane };
