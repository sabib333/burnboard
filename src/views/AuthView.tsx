...
export const AuthView: React.FC<AuthViewProps> = ({ onNavigateBack, onShowToast }) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        if (result.error) {
          ...
          try { localStorage.removeItem('burnboard_onboarding_completed'); } catch {}
          onShowToast('Welcome to BURNBOARD! 🔥', 'Account created. Check your email to confirm.');
          onNavigateBack();
        }
      } else {
        ...
        if (result.error) {
          ...
          onShowToast('Welcome back! 🔥', 'You are now logged in. Time to roast.');
          onNavigateBack();
        }
      }
    } finally {
